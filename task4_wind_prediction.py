...
# 重新执行聚类以获取标签（使用任务3相同的方法）
selected_features = ['WINDSPEED', 'TEMPERATURE', 'WINDDIRECTION', 'PRESSURE', 'WINDPOWER']
...
# 1. 数据准备：基于任务3的聚类结果
print("\n1. 数据准备：按聚类结果划分数据...")
X_all = df[['WINDSPEED', 'TEMPERATURE', 'WINDDIRECTION', 'PRESSURE']].values  # 特征
y_all = df['WINDPOWER'].values  # 目标变量
clusters = df['cluster'].values  # 聚类标签
...
for cluster_id in range(2):
    ...
    models[cluster_id]['SVR'] = svr_model
    ...
    models[cluster_id]['BP'] = bp_model
    
    print(f"    MAE: {bp_mae:.4f}, RMSE: {bp_rmse:.4f}, R2: {bp_r2:.4f}")

    # 2.3 LSTM神经网络（使用简化的时间序列方法）
    print(f"  2.3 训练LSTM神经网络模型...")
    ...
    if len(X_train) > 10:
        ...
        if len(X_train_seq) > 0 and len(X_test_seq) > 0:
            # 由于sklearn没有原生LSTM，我们使用其他方法模拟
            # 实际的LSTM需要tensorflow/keras，这里我们使用一种递归预测的方法
            ...
            models[cluster_id]['LSTM'] = lstm_model
            
            print(f"    MAE: {lstm_mae:.4f}, RMSE: {lstm_rmse:.4f}, R2: {lstm_r2:.4f}")
        else:
            print("    时间序列数据不足，跳过LSTM模型")
            results[cluster_id]['LSTM'] = None
            models[cluster_id]['LSTM'] = None
    else:
        print("    数据量不足，跳过LSTM模型")
        results[cluster_id]['LSTM'] = None
        models[cluster_id]['LSTM'] = None
    ...
    for cluster_id in results:
        for model_name in results[cluster_id]:
            if results[cluster_id][model_name] is not None:
                ...
                plt.savefig(f'task4_prediction_{cluster_id}_{model_name}.png', dpi=200, bbox_inches='tight')
                plt.close()
                print(f"已保存: task4_prediction_{cluster_id}_{model_name}.png")
